...
Job.prototype.runSequence = function() {
  ...
  async.mapSeries(
    this.makeRequests(this.opts.sequence),
    this.runRequest.bind(this, this.opts.sequence, requestLib.jar()),
    this.ranSequence);
};

Job.prototype.makeRequests = function (sequence) {
  var requests = [];
  ...
  for (var i = 0; i < sequence.length; i++) {
    ...
    for (var n = sequenceRequest.loop; n > 0; n--) {
      var r = Math.round(Math.random() * (sequenceRequest.requests.length - 1));
...
Job.prototype._prepareRequest = function (request, sequenceRequest) {
  request.sequenceRequest = sequenceRequest;
  return request;
};
